...
class ProjectController extends Controller
{
    ...
    public function create()
    {
        if (Auth::user()->isAbleTo('project create')) {
            ...
            $clients = User::where('created_by', '=', creatorId())->where('type', 'client')->where('workspace_id', getActiveWorkSpace())->get();
            return view('taskly::projects.create', compact('customFields', 'workspace_users', 'clients'));
        } else {
        ...
        if (Auth::user()->isAbleTo('project create')) {
            ...
            foreach ($userList as $email) {
                ...
                if ($registerUsers) {
                    ...
                    // dd($uArr);
                    // Log::info('Attempting to send New User email to: ' . $email);
                    // Log::info('New User Email Data: ', $uArr);
                    $smtp_error = EmailTemplate::sendEmailTemplate('New User', [$email], $uArr);
                    // Log::info('New User Email Response: ', (array)$smtp_error);
                ...
                if (!empty($registerUsers->mobile_no)) {
                    try {
                        $smsMessage = 'You have been added to the project: ' . $objProject->name . '. View it at: ' . route('projects.show', $objProject->id);
                        SmsHelper::sendSms($registerUsers->mobile_no, $smsMessage);
                    } catch (\Exception $e) {
                        // SMS sending failed, continue silently
                    }
                }
            }
            ...
            // Attach selected clients from the creation form
            if (!empty($request->clients_list)) {
                $company_settings = getCompanyAllSetting();
                foreach ($request->clients_list as $client_id) {
                    if (ClientProject::where('client_id', '=', $client_id)->where('project_id', '=', $objProject->id)->count() == 0) {
                        ...
                        // Send "Project Assigned" email to the client
                        $client = User::where('type', 'client')->where('id', $client_id)->first();
                        if ($client) {
                            $objProject->url = route('projects.show', $objProject->id);
                            $uArr = [
                                'name' => $client->name,
                                'project' => $objProject->name,
                                'project_creater_name' => $objUser->name,
                                'url' => $objProject->url,
                            ];
                            try {
                                // Log::info('Attempting to send Project Assigned email to client: ' . $client->email);
                                // Log::info('Email Data: ', $uArr);
                                EmailTemplate::sendEmailTemplate('Project Assigned', [$client->email], $uArr);
                            } catch (\Exception $e) {
                                // Log::error('Failed to send Project Assigned email to client: ' . $client->email . ' Error: ' . $e->getMessage());
                                // Email sending failed, continue silently
                            }
                        }

                        // Send SMS to the client
                        if ($client && !empty($client->mobile_no)) {
                            try {
                                $smsMessage = 'You have been added to the project: ' . $objProject->name . '. View it at: ' . route('projects.show', $objProject->id);
                                SmsHelper::sendSms($client->mobile_no, $smsMessage);
                            } catch (\Exception $e) {
                                // SMS sending failed, continue silently
                            }
                        }
    ...
    public function update(Request $request, Project $project)
    {
        if (Auth::user()->isAbleTo('project edit')) {
            ...
            // Check for Project Status Change
            if ($oldStatus != $project->status) {
                ...
                foreach ($projectUsers as $pUser) {
                    ...
                    if ($user) {
                        ...
                        try {
                            Log::info('Attempting to send Project Status Updated email to user: ' . $user->email);
                            EmailTemplate::sendEmailTemplate('Project Status Updated', [$user->email], $uArr);
                        } catch (\Exception $e) {
                            Log::error('Failed to send Project Status Updated email to user: ' . $user->email . ' Error: ' . $e->getMessage());
                        }
                        // SMS
                        if (!empty($user->mobile_no)) {
                            try {
                                $smsMessage = 'Project Status Updated: ' . $project->name . ' is now ' . $project->status . '. View it here: ' . route('projects.show', $project->id);
                                SmsHelper::sendSms($user->mobile_no, $smsMessage);
                            } catch (\Exception $e) {
                                Log::error('Failed to send Project Status Updated SMS to user: ' . $user->mobile_no . ' Error: ' . $e->getMessage());
                            }
                        }
                    }
                }
            ...
            foreach ($projectUsers as $pUser) {
                ...
                if ($user) {
                    ...
                    try {
                        // Log::info('Attempting to send Project Updated email to user: ' . $user->email);
                        // Log::info('Email Data: ', $uArr);
                        $resp = EmailTemplate::sendEmailTemplate('Project Updated', [$user->email], $uArr);
                        // Log::info('Email Response: ', (array)$resp);
                    } catch (\Exception $e) {
                        // Log::error('Failed to send Project Updated email to user: ' . $user->email . ' Error: ' . $e->getMessage());
                    }

                    // SMS
                    if (!empty($user->mobile_no)) {
                        try {
                            $smsMessage = 'Project ' . $project->name . ' has been updated. View it at: ' . route('projects.show', $project->id);
                            SmsHelper::sendSms($user->mobile_no, $smsMessage);
                        } catch (\Exception $e) {
                            // SMS sending failed, continue silently
                        }
            ...
            // Send Email and SMS to Clients
            $projectClients = ClientProject::where('project_id', $project->id)->get();
            foreach ($projectClients as $pClient) {
                $client = User::find($pClient->client_id);
                if ($client) {
                    // Email
                    $uArr = [
                        'name' => $client->name,
                    ...
                    try {
                        // Log::info('Attempting to send Project Updated email to client: ' . $client->email);
                        // Log::info('Email Data: ', $uArr);
                        $resp = EmailTemplate::sendEmailTemplate('Project Updated', [$client->email], $uArr);
                        // Log::info('Email Response: ', (array)$resp);
                    } catch (\Exception $e) {
                        // Log::error('Failed to send Project Updated email to client: ' . $client->email . ' Error: ' . $e->getMessage());
                    }
    ...
    public function inviteUser($user, $project, $permission)
    {
        ...
        if (!$is_invited) {
            ...
            if ($permission != 'company') {
                // Removed company setting check to force email sending
                $uArr = [
                    'name' => $user->name,
                    'project' => $project->name,
                ...
                try {
                    // Log::info('Attempting to send User Invited email to: ' . $user->email);
                    $resp = EmailTemplate::sendEmailTemplate('User Invited', [$user->email], $uArr);
                    // Log::info('User Invited Email Response: ', (array)$resp);
                } catch (\Exception $e) {
                    // Log::error('Failed to send User Invited email to: ' . $user->email . ' Error: ' . $e->getMessage());
    ...
    public function taskStore(Request $request, $projectID)
    {
        ...
        if ($project) {
            ...
            if ($stage) {
                ...
                // Send Email and SMS to Assigned Users
                    ...
                    foreach ($request->assign_to as $user_id) {
                        ...
                        if ($user) {
                            $uArr = [
                                ...
                                'task_url' => route('projects.task.board', $project->id),
                            ];

                            // Email
                            try {
                                Log::info('Attempting to send New Task Assigned email to: ' . $user->email);
                                EmailTemplate::sendEmailTemplate('New Task Assigned', [$user->email], $uArr);
                            } catch (\Exception $e) {
                                Log::error('Failed to send New Task Assigned email to: ' . $user->email . ' Error: ' . $e->getMessage());
                            }

                            // SMS
                            if (!empty($user->mobile_no)) {
                                try {
                                    $smsMessage = 'New Task Assigned: ' . $task->title . ' in Project: ' . $project->name . '. View it here: ' . route('projects.task.board', $project->id);
                                    SmsHelper::sendSms($user->mobile_no, $smsMessage);
                                } catch (\Exception $e) {
                                    Log::error('Failed to send New Task Assigned SMS to: ' . $user->mobile_no . ' Error: ' . $e->getMessage());
                                }
                            }
                        }
                    }
    ...
    public function TaskMember(Request $request, $projectID, $taskID)
    {
        ...
        if ($project) {
            ...
            $newAssignTo = $request->assign_to;
            $newlyAssigned = array_diff($newAssignTo, $oldAssignTo);
            if (!empty($newlyAssigned)) {
                foreach ($newlyAssigned as $user_id) {
                    $user = User::find($user_id);
                    if ($user) {
                        $uArr = [
                            'name' => $user->name,
                        ...
                        // Email
                        try {
                            Log::info('Attempting to send New Task Assigned email (from TaskMember) to: ' . $user->email);
                            EmailTemplate::sendEmailTemplate('New Task Assigned', [$user->email], $uArr);
                        } catch (\Exception $e) {
                            Log::error('Failed to send New Task Assigned email to: ' . $user->email . ' Error: ' . $e->getMessage());
                        }
                ...
                foreach ($assignees as $user_id) {
                    ...
                    if ($user) {
                        $uArr = [
                            'name' => $user->name,
                            'task_title' => $task->title,
                            'old_status' => Stage::find($oldStatus)->name ?? '',
                            'new_status' => Stage::find($request->stage_id)->name ?? '',
                            'project_name' => $project->name,
                            'project_url' => route('projects.show', $project->id),
                            'task_url' => route('projects.task.board', $project->id),
                        ];
                        ...
                        // Email
                        try {
                            Log::info('Attempting to send Task Status Updated email to: ' . $user->email);
                            EmailTemplate::sendEmailTemplate('Task Status Updated', [$user->email], $uArr);
                        } catch (\Exception $e) {
                            Log::error('Failed to send Task Status Updated email to: ' . $user->email . ' Error: ' . $e->getMessage());
                        }

                        // SMS
                        if (!empty($user->mobile_no)) {
                            try {
                                $smsMessage = 'Task Status Updated: ' . $task->title . ' is now ' . (Stage::find($request->stage_id)->name ?? '') . '. View it here: ' . route('projects.task.board', $project->id);
                                SmsHelper::sendSms($user->mobile_no, $smsMessage);
                            } catch (\Exception $e) {
                                Log::error('Failed to send Task Status Updated SMS to: ' . $user->mobile_no . ' Error: ' . $e->getMessage());
                            }
                        }
                    }
                }
            ...
            if (!empty($newlyAssigned)) {
                foreach ($newlyAssigned as $user_id) {
                    ...
                    if ($user) {
                        ...
                        // Email
                        try {
                            Log::info('Attempting to send New Task Assigned email (from Update) to: ' . $user->email);
                            EmailTemplate::sendEmailTemplate('New Task Assigned', [$user->email], $uArr);
                        } catch (\Exception $e) {
                            Log::error('Failed to send New Task Assigned email to: ' . $user->email . ' Error: ' . $e->getMessage());
                        }
            ...
            foreach ($assignees as $user_id) {
                ...
                if ($user) {
                    $uArr = [
                        ...
                        'new_status' => $new_status->name,
                        'project_name' => $project->name ?? '',
                        'project_url' => route('projects.show', $projectID),
                        'task_url' => route('projects.task.board', $projectID),
                    ];

                    // Email
                    try {
                        Log::info('Attempting to send Task Status Updated email (Order Update) to: ' . $user->email);
                        EmailTemplate::sendEmailTemplate('Task Status Updated', [$user->email], $uArr);
                    } catch (\Exception $e) {
                        Log::error('Failed to send Task Status Updated email to: ' . $user->email . ' Error: ' . $e->getMessage());
                    }
                    ...
                    if (!empty($user->mobile_no)) {
                        try {
                            $smsMessage = 'Task Status Updated: ' . $task->title . ' is now ' . $new_status->name . '. View it here: ' . route('projects.task.board', $projectID);
                            SmsHelper::sendSms($user->mobile_no, $smsMessage);
                        } catch (\Exception $e) {
                            Log::error('Failed to send Task Status Updated SMS to: ' . $user->mobile_no . ' Error: ' . $e->getMessage());
                        }
