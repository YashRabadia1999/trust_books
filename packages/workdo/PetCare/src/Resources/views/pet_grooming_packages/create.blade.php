...
{{ Form::open(['route' => 'pet.grooming.packages.store', 'method' => 'POST','class' => 'needs-validation', 'novalidate']) }}
<div class="modal-body">
    ...
    <div class="row">
        ...
        <div class="col-md-6 mb-3">
            <div class="form-group">
                ...
                {{ Form::number('total_package_amount', 0.0, ['class' => 'form-control', 'required' => 'required', 'id' => 'totalAmount', 'step' => '0.01', 'placeholder' => __('Enter Package Amount')]) }}
            </div>
        </div>        
    </div>
    ...
    <div class="col-md-12 mt-3">
        <div class="form-group">
            {{ Form::label('package_features', __('Package Features'), ['class' => 'form-label']) }}<x-required></x-required>
...
<script>
    $(document).ready(function() {
        ...
        // Repeter js
        // Add new service & vaccine
        $('.add_button').click(function() {
            const type = $(this).data('type'); // 'service' or 'vaccine'
            const wrapper = $('.field_wrapper[data-type="' + type + '"]');
            const firstField = wrapper.children().first();
            const fieldHTML = firstField.clone();
        ...
        $(document).on('change', '.service-select', function() {
            ...
            if (serviceId) {
                $.post("{{ route('get.pet.service.price') }}", {
                    ...
                    function(res) {
                        select.closest('.add-fild').find('.service-price').val(parseFloat(res
                            .servicePrice).toFixed(2));
                        calculateTotalAmount();
                    }).fail(function() {
                    console.error('Error:', 'Failed to fetch service price');
                });
            } else {
                select.closest('.add-fild').find('.service-price').val('0.00');
                calculateTotalAmount();
            }
        });
        ...
        $(document).on('change', '.vaccine-select', function() {
            ...
            if (vaccineId) {
                $.post("{{ route('get.pet.vaccine.price') }}", {
                    ...
                    function(res) {
                        select.closest('.add-fild').find('.vaccine-price').val(parseFloat(res
                            .vaccinePrice).toFixed(2));
                        calculateTotalAmount();
                    }).fail(function() {
                    console.error('Error:', 'Failed to fetch vaccine price');
                });
            } else {
                select.closest('.add-fild').find('.vaccine-price').val('0.00');
                calculateTotalAmount();
            }
        });

        // Recalculate service & vaccine price when it's manual change
