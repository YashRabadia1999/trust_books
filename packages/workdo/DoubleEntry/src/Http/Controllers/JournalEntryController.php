...
class JournalEntryController extends Controller
{
    ...
    public function create()
    {
        if (Auth::user()->isAbleTo('journalentry create')) {
            ...
            $subAccounts->leftjoin('chart_of_account_parents', 'chart_of_accounts.parent', 'chart_of_account_parents.account');
    ...
    public function update(Request $request, JournalEntry $journalEntry)
    {
        ...
        if (Auth::user()->isAbleTo('journalentry edit')) {
            if ($journalEntry->created_by == creatorId()) {
                ...
                $oldAccounts = $journalEntry->accounts;

                for ($i = 0; $i < count($oldAccounts); $i++) {
                    $journalItem = JournalItem::find($oldAccounts[$i]['id']);

                    $bankAccounts = [];
                    if (module_is_active('Account')) {
                        $bankAccounts = \Workdo\Account\Entities\BankAccount::where('chart_account_id', '=', $oldAccounts[$i]['account'])->get();
                    }
                    if (!empty($bankAccounts)) {
                        foreach ($bankAccounts as $bankAccount) {
                            $old_balance = $bankAccount->opening_balance;
        ...
        if (Auth::user()->isAbleTo('journalentry delete')) {
            if ($journalEntry->created_by == creatorId()) {
                ...
                foreach ($journalItems as $journalItem) {
                    ...
                    AddTransactionLine::where('reference_id', $journalEntry->id)->where('reference_sub_id', $journalItem->id)
