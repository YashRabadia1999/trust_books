...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form method="post" class="needs-validation needs-validation" action="{{ route('timetable.update', $timetables->id) }}" novalidate>
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-md-6 col-12 form-group">
                                ...
                                <div class="input-group">
                                    <input class="form-control" required="required" name="start_time" type="time" value="{{ $timetables->start_time }}" id="start_time">
                                    <span class="input-group-text">to</span>
                                    <input class="form-control" required="required" name="end_time" type="time" value="{{ $timetables->end_time }}" id="end_time">
                                </div>
                            </div>

                            <div class="form-group col-md-6">
                                <label class="form-label">{{ __('Class') }}</label><x-required></x-required>
                                {{ Form::select('class_id', $classRoom, $timetables->class_id, ['id' => 'class', 'class' => 'form-control class_id', 'placeholder' => 'Select Class' ,'required'=>'required']) }}
                            </div>
                                    ...
                                    <tbody class="subject-table-body">
                                        ...
                                        @foreach ($subjects as $key => $subject)
                                            <tr data-tr="{{ $key }}">
                                                ...
                                                @php
                                                    $allTimeData = json_decode($timetables->all_time, true);
                                                @endphp

                                                @foreach ($allTimeData as $dayName => $data)
                                                    <td data-td="{{ $loop->index }}">
                                                        <div class="input-group">
                                                            <input class="form-control time-input mb-2 rounded-1" name="first_time[{{ $key }}][{{ $dayName }}]" value="{{ $data[$subject->id]['first_time'] }}" type="time"  style="width: 100px;">
                                                            <input class="form-control time-input rounded-1" name="last_time[{{ $key }}][{{ $dayName }}]" value="{{ $data[$subject->id]['last_time'] }}"  type="time"  style="width: 100px;">
                                                        </div>
                                                    </td>
                                                @endforeach
                                                <td>
                                                    <input class="form-control total-hours" name="total_hours[{{ $key }}]" type="text" readonly>
                                                </td>
                                            </tr>
                                        @endforeach
                                        <tr>
