@extends('layouts.main')
@section('page-title', __('Create Exam'))
@section('title', __('Exam'))
@section('page-breadcrumb', __('Exams'))

@section('content')
<div class="row">
    <div class="col-sm-12">
        <div class="card">
            <div class="card-header d-flex align-items-center justify-content-between">
                <h5 class="mb-0">{{ __('Exam Form') }}</h5>
            </div>
            <div class="card-body">
                {{ Form::open(['route' => 'school.exam.store', 'method' => 'post', 'class'=>'needs-validation', 'novalidate']) }}
                <div class="row">
                    {{-- Exam Name --}}
                    <div class="col-md-6 mb-3">
                        <div class="form-group">
                            {{ Form::label('name', __('Exam Name'), ['class' => 'form-label']) }} <x-required></x-required>
                            {{ Form::text('name', null, [
                                'class'=>'form-control',
                                'placeholder'=>__('Enter Exam Name'),
                                'required'=>'required',
                                'autofocus'=>'autofocus'
                            ]) }}
                            <div class="invalid-feedback">
                                {{ __('Please enter exam name.') }}
                            </div>
                        </div>
                    </div>
                    {{-- Classroom --}}
                    <div class="col-md-6 mb-3">
                        <div class="form-group">
                            {{ Form::label('classroom_id', __('Classroom'), ['class' => 'form-label']) }} <x-required></x-required>
                            {!! Form::select('classroom_id', $classrooms, null, [
                                'class'=>'form-control',
                                'placeholder'=>__('Select Classroom'),
                                'required'=>'required'
                            ]) !!}
                        </div>
                    </div>
                    {{-- Academic Year --}}
                    <div class="col-md-6 mb-3">
                        <div class="form-group">
                            {{ Form::label('academic_year_id', __('Academic Year'), ['class' => 'form-label']) }} <x-required></x-required>
                            {!! Form::select('academic_year_id', $academicYears, null, [
                                'class'=>'form-control',
                                'placeholder'=>__('Select Academic Year'),
                                'required'=>'required'
                            ]) !!}
                        </div>
                    </div>

                    {{-- Term --}}
                    <div class="col-md-6 mb-3">
                        <div class="form-group">
                            {{ Form::label('term_id', __('Term'), ['class' => 'form-label']) }} <x-required></x-required>
                            {!! Form::select('term_id', $terms, null, [
                                'class'=>'form-control',
                                'placeholder'=>__('Select Term'),
                                'required'=>'required'
                            ]) !!}
                        </div>
                    </div>

                    {{-- Hidden JSON field --}}
                    <input type="hidden" name="students_json" id="students_json" />

                    {{-- Student marks table --}}
<div class="col-12 mb-3" id="student-section" style="display:none;">
    <label class="form-label fw-semibold">{{ __('Enter Exam Marks') }}</label>
    <div class="table-responsive">
        <table class="table table-bordered align-middle" id="students-table">
            <thead class="table-light">
                <tr>
                    <th>{{ __('Student') }}</th>
                    <th>{{ __('Student ID') }}</th>
                    <th>{{ __('Exam Marks') }}</th>
                    <th>{{ __('Assignment Marks') }}</th>
                    <th>{{ __('Total Marks') }}</th>
                    <th class="text-center">+</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>
                        <select class="form-control student-select">
                            <option value="">{{ __('Select Student') }}</option>
                            @foreach($students as $student)
                                <option value="{{ $student->id }}">{{ $student->name }} ({{ $student->id }})</option>
                            @endforeach
                        </select>
                    </td>
                    <td class="student-id-text"></td>
                    <td><input type="number" class="form-control student-marks" step="1" min="0"></td>
                    <td><input type="number" class="form-control assignment-marks" step="1" min="0"></td>
                    <td><input type="number" class="form-control total-marks" readonly></td>
                    <td class="text-center align-middle">
                        <a href="#" class="text-success add-row">+</a>
                        <a href="#" class="text-danger ms-2 remove-row" style="display:none;">&minus;</a>
                    </td>
                </tr>
            </tbody>
        </table>
    </div>
</div>


                </div>

                {{-- Form buttons --}}
                <div class="d-flex justify-content-end gap-2 mt-3">
                    <a href="{{ route('school.exam.index') }}" class="btn btn-light">{{ __('Cancel') }}</a>
                    {{ Form::submit(__('Save'), ['class'=>'btn btn-primary']) }}
                </div>
                {{ Form::close() }}
            </div>
        </div>
    </div>
</div>
@endsection

@push('scripts')
<script>
(function(){
    function bothSelected(){
        return $('select[name="academic_year_id"]').val() && $('select[name="term_id"]').val();
    }

    function toggleStudentSection(){
        if(bothSelected()) $('#student-section').slideDown(150);
        else $('#student-section').slideUp(150);
    }

    function buildStudentsJson(){
    var students = [];
    $('#students-table tbody tr').each(function(){
        var studentId = $(this).find('.student-select').val();
        if(!studentId) return;

        students.push({
            id: studentId,
            exam_marks: Number($(this).find('.student-marks').val()) || 0,
            assignment_marks: Number($(this).find('.assignment-marks').val()) || 0,
            total_marks: Number($(this).find('.total-marks').val()) || 0,
        });
    });

    $('#students_json').val(JSON.stringify({
        academic_year_id: Number($('select[name="academic_year_id"]').val()),
        term_id: Number($('select[name="term_id"]').val()),
        classroom_id: Number($('select[name="classroom_id"]').val()),  // Added classroom
        students: students
    }));
}

    // Update Student ID when selecting
    $(document).on('change', '.student-select', function(){
        var studentId = $(this).val();
        $(this).closest('tr').find('.student-id-text').text(studentId);
        calculateTotal($(this).closest('tr'));
    });

    // Calculate total marks
    function calculateTotal(tr){
        var exam = Number(tr.find('.student-marks').val()) || 0;
        var assignment = Number(tr.find('.assignment-marks').val()) || 0;
        tr.find('.total-marks').val(exam + assignment);
    }

    $(document).on('input', '.student-marks, .assignment-marks', function(){
        calculateTotal($(this).closest('tr'));
    });

    // Build JSON
    function buildStudentsJson(){
        var students = [];
        $('#students-table tbody tr').each(function(){
            var studentId = $(this).find('.student-select').val();
            if(!studentId) return;

            students.push({
                id: studentId,
                exam_marks: Number($(this).find('.student-marks').val()) || 0,
                assignment_marks: Number($(this).find('.assignment-marks').val()) || 0,
                total_marks: Number($(this).find('.total-marks').val()) || 0,
            });
        });

        $('#students_json').val(JSON.stringify({
            academic_year_id: Number($('select[name="academic_year_id"]').val()),
            term_id: Number($('select[name="term_id"]').val()),
            students: students
        }));
    }

    // Add / remove row
    function addRow(afterTr){
        var row = $('#students-table tbody tr:first').clone();
        row.find('select').val('');
        row.find('input').val('');
        row.find('.student-id-text').text('');
        if(afterTr) row.insertAfter(afterTr); else $('#students-table tbody').append(row);
        updateRemoveButtons();
    }

    function updateRemoveButtons(){
        var rows = $('#students-table tbody tr');
        rows.find('.remove-row').show();
        if(rows.length === 1) rows.find('.remove-row').hide();
    }

    $(document).on('click', '.add-row', function(e){ e.preventDefault(); addRow($(this).closest('tr')); });
    $(document).on('click', '.remove-row', function(e){ e.preventDefault(); $(this).closest('tr').remove(); updateRemoveButtons(); });

    $('form').on('submit', function(e){
        buildStudentsJson();
        if(!$('#students_json').val()){
            e.preventDefault();
            alert('Please add at least one student with marks.');
        }
    });

    updateRemoveButtons();
    toggleStudentSection();
    $(document).on('change', 'select[name="academic_year_id"], select[name="term_id"]', toggleStudentSection);
})();


</script>
@endpush
