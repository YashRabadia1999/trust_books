...
class BankAccountController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (Auth::user()->isAbleTo('bank account create')) {
            ...
            // Add conditional validation based on bank_type
            if ($request->bank_type === 'wallet') {
                $rules['wallet_type'] = 'required';
            }
    ...
    public function update(Request $request, $id)
    {
        if (Auth::user()->isAbleTo('bank account edit')) {
            ...
            if (
                BankAccount::where('payment_name', $request->payment_name)->where('id', '!=', $id)->where('workspace', getActiveWorkSpace())->where('created_by', creatorId())->exists()
            ) {
                return redirect()->route('bank-account.index')->with('error', __('This payment name already exists.'));
            }
