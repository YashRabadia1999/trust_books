...
class InvoiceController extends Controller
{
    ...
    function invoiceNumberApi($user_id)
    {
        $latest = company_setting('invoice_starting_number', $user_id, 4);
        if ($latest == null) {
            return 1;
        } else {
            return $latest;
    ...
    public function storeInvoiceApi(Request $request)
    {
        ...
        // Convert mobile number to +233 format
        $mobile = $request->mobile_no;
        if (substr($mobile, 0, 1) == '0') {
            $mobile = '+233' . substr($mobile, 1);
        } elseif (substr($mobile, 0, 3) == '233') {
            $mobile = '+' . $mobile;
        ...
        $invoice->invoice_id = $this->invoiceNumberApi($user_id); // you can use your existing method
        ...
        $invoice->category_id = 57;
        $invoice->invoice_template = null;
        $invoice->workspace = 4;
        $invoice->created_by = 3;
        $invoice->save();

        // Create invoice product
        $invoiceProduct = new InvoiceProduct();
        $invoiceProduct->invoice_id = $invoice->id;
        $invoiceProduct->product_type = 'product';
        $invoiceProduct->product_id = 5;
