...
class AuthenticatedSessionController extends Controller
{
    ...
    public function create(Request $request, $lang = '')
    {
        ...
        // Check for branded login token
        $company_settings = null;
        $company_id = null;
        if ($request->has('token')) {
            try {
                $user_id = decrypt($request->get('token'));
                $user = User::find($user_id);
                if ($user) {
                    $company_id = ($user->type == 'company') ? $user->id : $user->created_by;
                    if ($company_id) {
                        $company_settings = getCompanyAllSetting($company_id);
                    }
                }
            } catch (\Exception $e) {
                // Invalid token, use default settings
            }
        }

        return view('auth.login', compact('lang', 'company_settings', 'company_id'));
    }
