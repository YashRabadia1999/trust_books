...
class SettingsController extends Controller
{
    ...
    public function emailSettingGet($settings)
    {
        ...
        $email_notification_modules = Notification::where('type', 'mail')
            ->where(function ($query) use ($activatedModules) {
                $query->whereIn('module', $activatedModules)
    ...
    public function smsSettingGet($settings)
    {
        $activatedModules = ActivatedModule();
        $sms_notification_modules = Notification::where('type', 'sms')
            ->where(function ($query) use ($activatedModules) {
                $query->whereIn('module', $activatedModules)
                    ->orWhere('module', 'General');
            })
        ...
        $sms_notification_modules = array_unique($sms_notification_modules);

        $sms_notify = Notification::where('type', 'sms')->whereIn('module', $sms_notification_modules)->get(['module', 'action', 'permissions']);

        return view('sms.index', compact('settings', 'sms_notification_modules', 'sms_notify'));
    }
    ...
    public function mailStore(Request $request)
    {
        ...
        if (Auth::user()->isAbleTo('setting manage')) {
            ...
            return redirect()->back()->with('success', __('Mail Setting save sucessfully.'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function smsNotificationStore(Request $request)
    {
        // sms notification save
        if ($request->has('sms_noti')) {
            foreach ($request->sms_noti as $key => $notification) {
                // Define the data to be updated or inserted
                $data = [
                    'key' => $key . '_sms',
                    'workspace' => getActiveWorkSpace(),
        ...
        AdminSettingCacheForget();
        comapnySettingCacheForget();
        return redirect()->back()->with('success', __('SMS Notification Setting saved successfully.'));
    }
}
