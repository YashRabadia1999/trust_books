    ...
    function generateSubMenu($menuItems, $parent = null)
    {
        ...
        foreach ($filteredItems as $item) {
            ...
            if ($hasChildren) {
                ...
                $html .= '</li>';
                // ✅ Inject USSD Dashboard after School Dashboard
                if ($item['title'] == 'Dashboard') {
                    $html .= '<li class="dash-item">
                            <a href="' . route('ussd.dashboard') . '" class="dash-link">
                                USSD Dashboard
                            </a>
                          </li>';
                }
            ...
            if ($item['route'] == 'school-student.index') {
                $html .= '<li class="dash-item">
                            <a href="https://business.atdamss.com/schoolemployee" class="dash-link">
                                Teachers
                            </a>
                          </li>';
            }
...
if (!function_exists('SetConfigEmail')) {
    function SetConfigEmail($user_id = null, $workspace_id = null)
    {
        try {
            ...
            // ✅ Apply Gmail SMTP config dynamically
            config([
                'mail.default' => 'smtp',
                'mail.mailers.smtp.transport' => 'smtp',
                'mail.mailers.smtp.host' => $company_settings['mail_host'] ?? 'smtp.gmail.com',
                'mail.mailers.smtp.port' => $company_settings['mail_port'] ?? 587,
                'mail.mailers.smtp.encryption' => $company_settings['mail_encryption'] ?? 'tls',
                'mail.mailers.smtp.username' => $company_settings['mail_username'] ?? null,
                'mail.mailers.smtp.password' => $company_settings['mail_password'] ?? null, // MUST be Gmail App Password
                'mail.from.address' => $company_settings['mail_from_address'] ?? $company_settings['mail_username'] ?? null,
            ...
            // \Log::info('📧 Dynamic Gmail config applied', [
            ...
            \Log::error('SetConfigEmail error: ' . $e->getMessage());
            return false;
        }
    }
}
