{{ Form::model($workSpace, ['route' => ['workspace.update', $workSpace->id], 'method' => 'PUT', 'id' => 'workspace-edit-form']) }}
<div class="modal-body">
    ...
    <div class="form-group">
        ...
        {{ Form::text('slug', null, ['class' => 'form-control','required'=>'required','placeholder' => __('Enter Workspace Slug'), 'id'=>'slug']) }}
        <span id="slug-msg"></span>
    </div>

    {{-- SMS Sender ID --}}
    <div class="form-group">
        {{ Form::label('sms_sender_id', __('SMS Sender ID'), ['class' => 'col-form-label']) }}
        {{ Form::text('sms_sender_id', null, ['class' => 'form-control', 'maxlength' => 20, 'placeholder' => __('Enter SMS Sender ID')]) }}
        <small class="text-muted">{{ __('Enter your unique SMS Sender ID (max 20 characters)') }}</small>
    </div>
