    ...
    <div class="card">
        <div class="card-body">
            <div class="">
                ...
                @if (isset($company_settings) && !empty($company_settings))
                    <p class="text-muted small">{{ __('Welcome to') }}
                        {{ $company_settings['company_name'] ?? config('app.name') }}</p>
                @endif
            </div>
            <form method="POST" action="{{ route('login') }}" class="needs-validation" novalidate="" id="form_data">
                ...
                <div>
                    <div class="form-group mb-3">
                        <label class="form-label">{{ __('Email or Phone Number') }}</label>
                        <input id="email" type="text" class="form-control  @error('email') is-invalid @enderror"
                            name="email" value="{{ old('email') }}" placeholder="{{ __('Email or Phone Number') }}"
                            required autofocus>
                        @error('email')
