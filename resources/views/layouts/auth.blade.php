@php
    // Use company settings if available (from branded login), otherwise use admin settings
    $use_company_branding = isset($company_settings) && !empty($company_settings);
    $settings = $use_company_branding ? $company_settings : getAdminAllSetting();
    $admin_settings = $settings; // For backward compatibility

    $temp_lang = \App::getLocale('lang');
    if ($temp_lang == 'ar' || $temp_lang == 'he') {
        $rtl = 'on';
    } else {
        $rtl = isset($settings['site_rtl']) ? $settings['site_rtl'] : 'off';
    }
...
<body class="{{ $themeColor }}">
                    ...
                    <div class="container">
                        <div class="navbar-brand">
                            <a class="navbar-brand" href="{{ url('/') }}">
                                @php
                                    // Use company logo for branded login, otherwise use sidebar_logo()
                                    if ($use_company_branding) {
                                        $dark_mode =
                                            (isset($company_settings['cust_darklayout'])
                                                ? $company_settings['cust_darklayout']
                                                : 'off') == 'on';
                                        $logo_key = $dark_mode ? 'logo_light' : 'logo_dark';
                                        $default_logo = $dark_mode
                                            ? 'uploads/logo/logo_light.png'
                                            : 'uploads/logo/logo_dark.png';

                                        if (
                                            !empty($company_settings[$logo_key]) &&
                                            check_file($company_settings[$logo_key])
                                        ) {
                                            $auth_logo = $company_settings[$logo_key];
                                        } else {
                                            $auth_logo = $default_logo;
                                        }
                                    } else {
                                        $auth_logo = sidebar_logo();
                                    }
                                @endphp
                                <img src="{{ get_file($auth_logo) }}{{ '?' . time() }}"
    ...
    <!-- Block external verify.js on login page only -->
    <script src="{{ asset('js/block-verify.js') }}"></script>
